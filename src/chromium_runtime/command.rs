...
/// Renderer-independent command boundary for the Chromium compatibility host.
///
/// No CEF browser, render handler, process handle, temporary accelerated-paint
/// handle, or platform window object may cross this channel.
pub(crate) enum ChromiumCommand {
    CreateView {
        config: ViewConfig,
        reply: Reply<ViewId>,
    },
...
impl ChromiumCommand {
    pub(crate) fn fail(self, message: impl Into<String>) {
        let message = message.into();
        let error = || EngineError::Internal(message.clone());
